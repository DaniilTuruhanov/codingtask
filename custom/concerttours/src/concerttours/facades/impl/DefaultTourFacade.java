...
import concerttours.enums.OpenOrClose;
...
public class DefaultTourFacade implements TourFacade
{
    ...
    public TourData getTourDetails(final String tourId)
    {
        ...
        if (product.getVariants() != null)
        {
            for (final VariantProductModel variant : product.getVariants())
            {
                if (variant instanceof ConcertModel)
                {
                    ...
                    summary.setType(concert.getOpenOrClose() == OpenOrClose.OPENAIR.OPENAIR ? "Outdoors" : "Indoors");
        ...
        ProducerModel producer = product.getProducer();
        ...
        tourData.setProducer(producer.getName());
        return tourData;
    }
    @Required
